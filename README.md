# Norway vs England - Pre Match Analysis

A pre-match football data analysis project for the FIFA World Cup 2026 Quarter-final between Norway and England.

## Objectives

- Analyze both teams using football data.
- Compare player performances.
- Identify tactical strengths and weaknesses.
- Build a data-driven pre-match report using Python.

## Tools

- Python
- Pandas
- Matplotlib
- mplsoccer

> This project is part of my Football Data Analytics portfolio.
## Dataset

Competition:
- FIFA World Cup 2026 Qualification Europe

Teams:
- Norway (NOR)
- England (ENG)

Matches analyzed:
- Norway: 8
- England: 8
- Total: 16


## Team Performance

| Team | Matches | Goals For | Goals Against | Goal Difference |
|---|---:|---:|---:|---:|
| Norway | 8 | 37 | 5 | +32 |
| England | 8 | 22 | 0 | +22 |


## Player Highlights

## Norway (NOR)

- Erling Haaland: 16 goals, 2 assists
- Martin Ødegaard: 7 assists, 27 key passes
- Alexander Sørloth: 5 goals, 2 assists


## England (ENG)

- Harry Kane: 8 goals
- Declan Rice: 1 goal, 4 assists
- Jude Bellingham: 1 assist


## Statistical Comparison

### Possession & Passing

| Team | Possession | Pass Accuracy |
|---|---:|---:|
| Norway | 57.5% | 89.2% |
| England | 74.4% | 92.0% |


### Shooting

| Team | Total Shots | Shots on Goal | Inside Box Shots |
|---|---:|---:|---:|
| Norway | 155 | 61 | 121 |
| England | 151 | 63 | 102 |


## Tactical Analysis

Norway:
- Direct attacking style
- High finishing efficiency
- Haaland focused attacking threat

England:
- Possession dominance
- Strong passing structure
- Defensive stability


## Match Prediction

England is expected to control possession, while Norway has stronger individual attacking power.

Prediction:

**Norway 1-2 England**